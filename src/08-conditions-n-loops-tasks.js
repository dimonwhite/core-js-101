...
function getFactorial(n) {
  return [...Array(n)].reduce((fact, item, i) => fact * (i + 1), 1);
}
...
function isCreditCardNumber(cnn) {
  const value = cnn.toString();
  let nCheck = 0;
  let bEven = !(value.length % 2);
...
function getDigitalRoot(num) {
  const result = num.toString().split('').reduce((acc, item) => acc + +item, 0);
